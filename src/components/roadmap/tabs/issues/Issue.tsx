    ...
    return (
      <div className='w-full flex my-6 relative'>
        <div className='w-10 h-10 '>
          <img draggable="false" src={openIssue ? circledot : circle}
               alt='issue icon'/>
        </div>
        <div>
          <button
          ...
          <div
            className='font-medium font-roboto-text text-secondary text-sm mt-1'>
            by <a href={`/profile/${authorId}`}>{author}</a>
          </div>
        </div>
        {gotPermission && (<div className='absolute right-0 h-full'>
          <button
            type='button'
